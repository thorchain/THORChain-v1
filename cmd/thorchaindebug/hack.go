...
func runHackCmd(_ *cobra.Command, args []string) error {
	...
	db, err := dbm.NewGoLevelDB("thorchain", dataDir)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	app := NewThorchainApp(logger, db, baseapp.SetPruning(viper.GetString("pruning")))
	...
	// XXX: start hacking!
	//----------------------------------------------------
	// eg. gaia-6001 testnet bug (from cosmos)
