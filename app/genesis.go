...
func ThorchainAppInit() server.AppInit {
	...
	return server.AppInit{
		...
		AppGenTx:         ThorchainAppGenTx,
		AppGenState:      ThorchainAppGenStateJSON,
	}
}

// simple genesis tx
type ThorchainGenTx struct {
...
func ThorchainAppGenTx(cdc *wire.Codec, pk crypto.PubKey, genTxConfig config.GenTx) (
	...
	appGenTx, _, validator, err = ThorchainAppGenTxNF(cdc, pk, addr, genTxConfig.Name)
	return
}

// Generate a thorchain genesis transaction without flags
func ThorchainAppGenTxNF(cdc *wire.Codec, pk crypto.PubKey, addr sdk.AccAddress, name string) (
	appGenTx, cliPrint json.RawMessage, validator tmtypes.GenesisValidator, err error) {

	var bz []byte
	thorchainGenTx := ThorchainGenTx{
		Name:    name,
		Address: addr,
		PubKey:  sdk.MustBech32ifyAccPub(pk),
	}
	bz, err = wire.MarshalJSONIndent(cdc, thorchainGenTx)
	if err != nil {
		return
	}
...
// Create the core parameters for genesis initialization for thorchain
